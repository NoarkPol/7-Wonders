...
namespace SevenWondersDuel {
    ...
    void DraftWonderCommand::execute(GameController& controller) {
        auto& model = *controller.model; // Friend access
        ...
        if (it != pool.end()) {
            ...
            // Logic from handleDraftWonder
            bool shouldSwitch = true;
            if (controller.draftTurnCount == 1) shouldSwitch = false;

            controller.draftTurnCount++;

            if (model.getDraftPool().empty()) {
                if (controller.currentState == GameState::WONDER_DRAFT_PHASE_1) {
                    controller.setState(GameState::WONDER_DRAFT_PHASE_2);
                    controller.dealWondersToDraft();
                    controller.draftTurnCount = 0;
    ...
    void BuildCardCommand::execute(GameController& controller) {
        ...
        // Validation happened before, so just cost calculation for paying
        auto costInfo = currPlayer->calculateCost(targetCard->getCost(), *opponent, targetCard->getType());
        
        // Chain logic check again for safety/consistency (or trust validation)
        bool isChain = false;
        ...
        if (controller.currentState == GameState::AGE_PLAY_PHASE) {
    ...
    void SelectProgressTokenCommand::execute(GameController& controller) {
        ...
        bool success = false;
        if (controller.currentState == GameState::WAITING_FOR_TOKEN_SELECTION_PAIR) {
            success = model.getBoardMut()->removeAvailableProgressToken(token);
        } else if (controller.currentState == GameState::WAITING_FOR_TOKEN_SELECTION_LIB) {
